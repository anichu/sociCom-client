...
const Post = ({
	...
	const fetchTotalCount = () => {
		fetch(`https://socicom-server-anichu.vercel.app/post/like/total/${_id}`)
			.then((res) => res.json())
			.then((data) => {
				console.log(data);
				setLikeCount(data);
		...
		try {
			if (isLike?.like !== "like") {
				const { data } = await axios.post(
					`https://socicom-server-anichu.vercel.app/post/like/${_id}`,
					createReaction
				);
				setIsLike(data);
				console.log(data);
	...
	useEffect(() => {
		fetch(`https://socicom-server-anichu.vercel.app/user?email=${post?.email}`)
			.then((res) => res.json())
			.then((data) => {
				// console.log(data);
				setPostUser(data);
	...
	const deletePostHandler = async (id) => {
		...
		try {
			const { data } = axios.delete(
				`https://socicom-server-anichu.vercel.app/post/${id}`
			);

			toast.success("The post deleted successfully");
			const fileteredMyposts = myposts.filter((mypost) => mypost._id !== id);
			setMyposts(fileteredMyposts);
			// fetchMyPost();
			console.log(data);
		} catch (error) {
			console.log(error);
	...
	return (
		<div className="w-full p-2">
			<div className="sm:w-3/4 w-full relative bg-indigo-800  my-4 rounded-md sm:p-10 p-3 sm:mx-auto">
				{profile && user?.email === post?.email && (
					<span
						onClick={() => deletePostHandler(post?._id)}
						className="absolute cursor-pointer top-2 right-2 mb-1"
					>
						<FaTrash className="w-6 h-6  text-red-600"></FaTrash>
					</span>
				)}

				<div className="flex w-full bg-indigo-800 items-center justify-between">
					<div className="flex items-center">
						...
						<div className="profile-info sm:ml-3 ml-2">
